...
'use strict';
{
    window.SelectFilter = {
        init: function(field_id, field_name, is_stacked) {
            ...
            quickElement(
                'span', search_filter_label_chosen, '',
                'class', 'help-tooltip search-label-icon',
                'title', interpolate(gettext("Type into this box to filter down the list of available %s."), [field_name])
            );
            ...
            filter_input_chosen.addEventListener('keyup', function(e) {
                SelectFilter.filter_key_up_chosen(e, field_id);
            });
            ...
            selector_available.addEventListener('change', function(e) {
                if (e.target.tagName === 'SELECT') {
                    SelectFilter.refresh_icons(field_id);
                }
            });
        ...
        filter_key_up_chosen: function(event, field_id) {
            const from = document.getElementById(field_id + '_to');
            const temp = from.selectedIndex;
